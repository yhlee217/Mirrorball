...
1. **HandSOS 크롤링 ToS·자격증명 위탁** — 최대 법적/운영 리스크. 완화: 명시적 동의·약관, 자격증명 Vault 격리, 공식 연동 병행 추진, 세션 재사용, 재인증 UX. → *공식 API 존재 여부 확인 필요.*
...
- [ ] **HandSOS 공식 연동/API 존재 여부 확인** (있으면 크롤링 리스크 대폭 감소 — 첫 단추)
