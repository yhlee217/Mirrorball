...
> **데이터는 어디에? (별도 DB 불필요)**
> 고객·거래·메모·가족관계는 전부 로컬 파일에 저장됩니다 —
> `clients/{slug}/customers/*.yaml` + `records.yaml`(원본), `dist_app/{slug}.json`(앱이 읽음).
> SQL DB·서버를 띄울 필요가 없고, PII 가 그 PC 밖으로 나가지 않습니다.
> 중앙 관리·다기기 실시간 동기화가 필요해지는 규모에서 그때 DB(Supabase/자체호스팅 등)를 검토.
