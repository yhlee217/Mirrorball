...
준비물: **비즈니스 인스타 계정 1개**(아래 "렌즈" 참고) · 페이스북 계정(없으면 무료 생성) · PC 브라우저
※ Meta 화면·메뉴 이름은 수시로 바뀜 — 이름이 다르면 비슷한 항목을 찾으면 된다.

> **"렌즈 계정" — 꼭 살롱 계정일 필요 없음.** API 를 호출하는 계정(렌즈)과 측정 대상은 분리된다.
> 해시태그 트렌드·경쟁사·살롱 **공개** 데이터(팔로워·게시물·인게이지먼트)는 **컨시어지 본인 비즈니스 계정**
> 하나로 다 본다(여러 클라이언트를 계정 1개로 모니터링). 살롱 **비공개** 인사이트(도달·노출)·정확한
> 멘션 목록이 필요할 때만 살롱 계정을 렌즈로 쓰고 `lens_is_salon: true`. → 아래 STEP 은 "렌즈로 쓸 계정"
> 기준으로 따라가면 된다(컨시어지 계정이든 살롱 계정이든 절차 동일).
